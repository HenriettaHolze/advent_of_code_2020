import regex as re

INPUT = '''ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
byr:1937 iyr:2017 cid:147 hgt:183cm

iyr:2013 ecl:amb cid:350 eyr:2023 pid:028048884
hcl:#cfa07d byr:1929

hcl:#ae17e1 iyr:2013
eyr:2024
ecl:brn pid:760753108 byr:1931
hgt:179cm

hcl:#cfa07d eyr:2025 pid:166559648
iyr:2011 ecl:brn hgt:59in'''.split('\n\n')


INPUT_invalid = '''eyr:1972 cid:100
hcl:#18171d ecl:amb hgt:170 pid:186cm iyr:2018 byr:1926

iyr:2019
hcl:#602927 eyr:1967 hgt:170cm
ecl:grn pid:012533040 byr:1946

hcl:dab227 iyr:2012
ecl:brn hgt:182cm pid:021572410 eyr:2020 byr:1992 cid:277

hgt:59cm ecl:zzz
eyr:2038 hcl:74454a iyr:2023
pid:3556412378 byr:2007'''.split('\n\n')

INPUT_valid = '''pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980
hcl:#623a2f

eyr:2029 ecl:blu cid:129 byr:1989
iyr:2014 pid:896056539 hcl:#a97842 hgt:165cm

hcl:#888785
hgt:164cm byr:2001 iyr:2015 cid:88
pid:545766238 ecl:hzl
eyr:2022

iyr:2010 hgt:158cm hcl:#b6652a ecl:blu byr:1944 eyr:2021 pid:093154719'''.split('\n\n')


def check_passports(passports):
    '''Check passports,
    return number of valid passports'''
    valid_passports = 0

    for passport in passports:
        fields = len(passport.split()) 
        # cid is optional
        if 'cid:' not in passport: 
            fields += 1

        if fields >= 8:
            valid_passports += 1

    return valid_passports


assert(check_passports(INPUT) == 2)


with open('04.txt', 'r') as o:
    # split passports on blank lines
    passports = o.read().split('\n\n')

print(check_passports(passports))

# ------------------------------------

def check_field(field):
    field_validation = {
        # if it is not an int, will return false, must be > 999 to be 4 digits
        'byr': lambda year: 1920 <= int(year) <= 2002,
        'iyr': lambda year: 2010 <= int(year) <= 2020,
        'eyr': lambda year: 2020 <= int(year) <= 2030,
        'hgt': lambda height: (height[-2:] == 'cm' and 150 <= int(height[:-2]) <= 193) or (height[-2:] == 'in' and 59 <= int(height[:-2]) <= 76),
        'hcl': lambda hcl: hcl[0] == '#' and bool(re.search('^#[0-9a-f]{6}$', hcl)),
        'ecl': lambda ecl: bool(re.search('^amb|blu|brn|gry|grn|hzl|oth$', ecl)),
        'pid': lambda pid: bool(re.search('^[0-9]{9}$', pid))
    }
    key, value = field.split(':')
    # cid does not have to be checked
    if key == 'cid':
        return True
    try:
        return field_validation[key](value)
    except KeyError as e:
        print(e)
        return False


def check_passports_field_validation(passports):
    '''Check passports,
    return number of valid passports'''
    valid_passports = 0

    for passport in passports:
        fields = passport.split()
        field_number = len(fields)
        # cid is optional
        if 'cid:' not in passport: 
            field_number += 1

        if field_number >= 8:
            valid = True
            for field in fields:
                if not check_field(field):
                    valid = False
            if valid: 
                valid_passports += 1

    return valid_passports


assert(check_passports_field_validation(INPUT_invalid) == 0)
assert(check_passports_field_validation(INPUT_valid) == len(INPUT_valid))

print(check_passports_field_validation(passports))
